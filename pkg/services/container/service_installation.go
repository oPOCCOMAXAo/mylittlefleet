...
func (s *Service) initInternalContainers(ctx context.Context) error {
	byName, err := s.GetContainersMapByName(ctx,
		ContainerNameSelf,
		ContainerNameReverseProxy,
	)
	if err != nil {
		return err
	}

	if _, ok := byName[ContainerNameSelf]; !ok {
		err = s.repo.CreateContainer(ctx, &models.Container{
	...
	if _, ok := byName[ContainerNameReverseProxy]; !ok {
		err = s.repo.CreateContainer(ctx, &models.Container{
...
func (s *Service) checkIfContainerIsCurrentInstallation(
	...
	s.logger.InfoContext(ctx, "Checking container",
		slog.Any("container", container),
	)
